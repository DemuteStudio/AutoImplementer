...
TScriptInterface<IMetaSoundDocumentInterface> UGISB_MetasoundPatchBuilder::BuildSimpleSoundNode(UGisbImportContainerSimpleSound* simpleSound, const FString& Name)
{
	...
	FMetaSoundNodeHandle wavePlayerHandle = builder->AddNodeByClassName(
	  isStereo ? *UGISB_MetasoundNodeLibrary::WavePlayerStereoNode : *UGISB_MetasoundNodeLibrary::WavePlayerMonoNode,
	  result
	);
...
)
{
	...
	// CASE 2: Multiple children (2-8) - use UGISB_MetasoundNodeLibrary::GisbRandomNode + Mixer + TriggerAny
	if (childResults.Num() <= 8)
	{
		// Add UGISB_MetasoundNodeLibrary::GisbRandomNode for selection
		...
		// Configure UGISB_MetasoundNodeLibrary::GisbRandomNode
		...
		//Calculating which nodes to use depending on children amount
		...
		for (int32 i = 0; i < childResults.Num(); i++)
		{
			...
			if (bIsStereo)
			{
				FMetaSoundBuilderNodeOutputHandle childAudioRight = childAudioLeft; //Connecting to both outputs if this child is mono
	...
	setupAttributes(builder, randomContainer, bIsStereo, &triggerInputHandle, audioLeftHandle, audioRightHandle, &Layout);
...
)
{
	...
	// CASE 1: Multiple children (2-8) - use Mixer + TriggerAccumulate (wait for all)
	if (childResults.Num() <= 8)
	{
		int32 adjustedIndex = childResults.Num() < 2 ? 0 : childResults.Num() - 2;
		...
		FMetaSoundNodeHandle accumulateHandle = builder->AddNodeByClassName(
			*((*UGISB_MetasoundNodeLibrary::TriggerAccumulateNodes)[childResults.Num() - 1]), // -1 because array starts at 1 input
			result
		);
		...
		for (int32 i = 0; i < childResults.Num(); i++)
		{
			...
			if (bIsStereo)
			{
				FMetaSoundBuilderNodeOutputHandle childAudioRight = childAudioLeft; //If child is mono in stereo parent, route mono channel to both stereo channels
				if (bisChildStereo)
				{
					childAudioRight = builder->FindNodeOutputByName(childPatchNode, TEXT("Audio Right"), result);
				}
	...
	{
		// CASE 2: >8 children - hierarchical cascading (TODO)
		UE_LOG(LogTemp, Error, TEXT("BuildBlendNode: >8 children not yet implemented for %s"), *Name);
		return nullptr;
	}

	// Stage 6: Apply Audio Effects Chain
	setupAttributes(builder, blendContainer, bIsStereo, &triggerInputHandle, audioLeftHandle, audioRightHandle, &Layout);
...
)
{
	...
	// CASE 1: Multiple children (1-8) - use UGISB_MetasoundNodeLibrary::GisbSwitchNode + Mixer + TriggerAny
	if (childResults.Num() <= 8)
	{
		// Add UGISB_MetasoundNodeLibrary::GisbSwitchNode for parameter-driven selection
		...
		// Configure UGISB_MetasoundNodeLibrary::GisbSwitchNode
		...
		// Set case names (assuming UGISB_MetasoundNodeLibrary::GisbSwitchNode accepts string array)
		...
		for (int32 i = 0; i < childResults.Num(); i++)
		{
			...
			if (bIsStereo)
			{
				FMetaSoundBuilderNodeOutputHandle childAudioRight = childAudioLeft; //if child is mono in stereo parent, route signal to both channels
				if (bisChildStereo)
				{
					childAudioRight = builder->FindNodeOutputByName(childPatchNode, TEXT("Audio Right"), result);
				}
	...
	// Stage 6: Apply Audio Effects Chain
	setupAttributes(builder, switchContainer, bIsStereo, &triggerInputHandle, audioLeftHandle, audioRightHandle, &Layout);
...
void UGISB_MetasoundPatchBuilder::setupProbability(UMetaSoundPatchBuilder* builder, float probability, FMetaSoundBuilderNodeOutputHandle& playTrigger, FMetaSoundBuilderNodeInputHandle& onFinished, GisbMetasoundLayoutManager* Layout)
{
...
void UGISB_MetasoundPatchBuilder::setupAttributes(UMetaSoundPatchBuilder* builder, UGisbImportContainerBase* container, const bool isStereo, FMetaSoundBuilderNodeOutputHandle* executionHandle, FMetaSoundBuilderNodeOutputHandle& firstAudioHandle, FMetaSoundBuilderNodeOutputHandle& secondAudioHandle, GisbMetasoundLayoutManager* Layout)
{
	// Apply Volume effect if configured
	if (container->VolumeDB.randomize || container->VolumeDB.value != 0)
	{
		ConnectVolume(builder, container->VolumeDB, isStereo, executionHandle, firstAudioHandle, secondAudioHandle, Layout);
	}

	// Apply Pitch effect if configured
	if (container->Pitch.randomize || container->Pitch.value != 0)
	{
		ConnectPitch(builder, container->Pitch, isStereo, executionHandle, firstAudioHandle, secondAudioHandle, Layout);
	}

	// Apply Lowpass effect if configured
	if (container->Lowpass.randomize || container->Lowpass.value != 0)
	{
		ConnectLowpass(builder, container->Lowpass, isStereo, executionHandle, firstAudioHandle, secondAudioHandle, Layout);
	}
}

void UGISB_MetasoundPatchBuilder::ConnectProbabilityNode(UMetaSoundPatchBuilder* builder, float probability, FMetaSoundBuilderNodeOutputHandle& executionHandle, FMetaSoundBuilderNodeInputHandle& finishHandle, GisbMetasoundLayoutManager* Layout)
{
	EMetaSoundBuilderResult result;
	...
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to add Trigger Filter node"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Probability input"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Trigger input"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to set Probability value"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to connect execution trigger"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Heads output"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Tails output"));
		return;
	}
	...
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to add Trigger Any node"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Trigger Any In 1 input"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to connect Tails to Trigger Any"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Trigger Any Out output"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to connect Trigger Any to finish"));
		return;
	}
	...
	if (result != EMetaSoundBuilderResult::Succeeded)
	{
		UE_LOG(LogTemp, Error, TEXT("ConnectUGISB_MetasoundNodeLibrary::ProbabilityNode: FAILED to find Trigger Any In 0 input"));
		return;
	}
...
void UGISB_MetasoundPatchBuilder::ConnectLowpass(UMetaSoundPatchBuilder* builder, const FGisbLowPass lowpass, const bool isStereo, FMetaSoundBuilderNodeOutputHandle* executionHandle, FMetaSoundBuilderNodeOutputHandle& firstAudioHandle, FMetaSoundBuilderNodeOutputHandle& secondAudioHandle, GisbMetasoundLayoutManager* Layout)
{
	EMetaSoundBuilderResult result;
...
bool UGISB_MetasoundPatchBuilder::isStereo(UGisbImportContainerBase* container)
{
	if (UGisbImportContainerSimpleSound* SimpleSound = Cast<UGisbImportContainerSimpleSound>(container))
	{
